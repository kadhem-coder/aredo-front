...
import { Plus, XCircle, Grid3X3, List, Newspaper, Filter } from "lucide-react"
import {
  useGetNewsQuery,
...
const NewsPage = () => {
  ...
  // State for filters
  const [filters, setFilters] = useState<GetNewsRequest>({
    page: 1,
    page_size: 12,
  })

  // API calls
  const { data: newsData, isLoading, error, refetch } = useGetNewsQuery(filters)
  ...
  const handleDeleteNews = async (id: string, title: string) => {
    ...
    try {
      ...
      toast.error(error?.data?.message || "فشل في حذف الخبر")
    }
  }
  ...
  const totalPages = newsData?.count ? Math.ceil(newsData.count / (filters.page_size || 12)) : 0
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <Card className="p-6 text-center max-w-md bg-card backdrop-blur-sm border border-border/50 shadow-2xl">
          ...
          <p className="text-muted-foreground mb-4">حدث خطأ أثناء تحميل قائمة الأخبار</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gradient">إدارة الأخبار</h1>
            <p className="text-muted-foreground flex items-center gap-1">
              <Filter className="w-4 h-4 text-primary-solid" />
              إدارة وتنظيم الأخبار في النظام
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button 
            onClick={() => router.push(`/dashboard/news/createOrUpdate`)}
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between text-sm">
              <div className="text-primary-solid font-medium">
                عرض {((currentPage - 1) * (filters.page_size || 12)) + 1} إلى {Math.min(currentPage * (filters.page_size || 12), totalCount)} من أصل {totalCount} خبر
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {newsList.map((news: News) => (
                <NewsGrid
                  key={news.id}
                  news={news}
                  onViewDetails={() => handleViewDetails(news.id)}
                  onEdit={() => handleEdit(news)}
                  onDelete={() => handleDeleteNews(news.id, news.title)}
                />
              ))}
            </div>
          ) : (
      ...
      {!isLoading && newsList.length === 0 && (
        <div className="flex items-center justify-center min-h-[300px]">
          <Card className="p-8 text-center max-w-md bg-card backdrop-blur-sm border-border/50 shadow-2xl">
            ...
            <h3 className="text-lg font-semibold mb-2 text-card-foreground">لا توجد أخبار</h3>
            <p className="text-muted-foreground mb-4">لم يتم العثور على أي أخبار في النظام</p>
            <Button 
              onClick={() => router.push(`/dashboard/news/createOrUpdate`)}
